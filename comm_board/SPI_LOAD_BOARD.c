...
#define SS_LOAD_BOARD        LATAbits.LATA12      // slave select pin
...
int getFloatLoadBoard(float* result){
    ...
    for(i=0; i<4; ++i) {
    // for(i=0; i<4; ++i) {
        if(!sendLoadBoardByte(SPI_REQUEST_DATA, dataIn + i)) {
            return 0;
        }
        delay(10);
    }

   *result = raw2ieee(dataIn);
   return 1;
}
...
int sendCommandToLoadBoard(int command){
    uint8_t response;
    int send_successful = sendLoadBoardByte(command, &response);
    return send_successful && (response == 0);
}

int getFloatForCommand(uint8_t command, float* result) {
    int command_send = sendCommandToLoadBoard(command);
